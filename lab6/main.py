s_box = [
    
]


def gmul(a, b):
    p = 0
    for _ in range(8):
        if b & 1:
            p ^= a
        hi_bit_set = a & 0x80
        a <<= 1
        if hi_bit_set:
            a ^= 0x1b
        b >>= 1
    return p % 256


def sub_bytes(state):
    for i in range(4):
        for j in range(4):
            state[i][j] = s_box[state[i][j]]


# Зсув рядків у ShiftRows
def shift_rows(state):
    state[1] = state[1][1:] + state[1][:1]
    state[2] = state[2][2:] + state[2][:2]
    state[3] = state[3][3:] + state[3][:3]


# Помішання стовпців у MixColumns
def mix_columns(state):
    for i in range(4):
        s0 = gmul(0x02, state[0][i]) ^ gmul(0x03, state[1][i]) ^ state[2][i] ^ state[3][i]
        s1 = state[0][i] ^ gmul(0x02, state[1][i]) ^ gmul(0x03, state[2][i]) ^ state[3][i]
        s2 = state[0][i] ^ state[1][i] ^ gmul(0x02, state[2][i]) ^ gmul(0x03, state[3][i])
        s3 = gmul(0x03, state[0][i]) ^ state[1][i] ^ state[2][i] ^ gmul(0x02, state[3][i])
        state[0][i], state[1][i], state[2][i], state[3][i] = s0, s1, s2, s3


# Змішування байтів у AddRoundKey
def add_round_key(state, round_key):
    for i in range(4):
        for j in range(4):
            state[i][j] ^= round_key[i][j]


# Розгортання ключа для одного раунду
def key_expansion(key, round_constant):
    key = key[1:] + key[:1]
    key = [s_box[b] for b in key]
    key[0] ^= round_constant
    return key


# Основна функція для одного раунду AES
def aes_round(state, round_key):
    sub_bytes(state)
    shift_rows(state)
    mix_columns(state)
    add_round_key(state, round_key)


# Операція розгортання ключа
def key_schedule(initial_key):
    round_keys = [initial_key[i:i + 4] for i in range(0, 16, 4)]
    for round_constant in range(1, 11):
        round_keys.append(key_expansion(round_keys[-1], round_constant))
    return round_keys


# Приклад використання
plaintext = [[0x32, 0x88, 0x31, 0xe0],
             [0x43, 0x5a, 0x31, 0x37],
             [0xf6, 0x30, 0x98, 0x07],
             [0xa8, 0x8d, 0xa2, 0x34]]

key = [[0x2b, 0x28, 0xab, 0x09],
       [0x7e, 0xae, 0xf7, 0xcf],
       [0x15, 0xd2, 0x15, 0x4f],
       [0x16, 0xa6, 0x88, 0x3c]]

round_keys = key_schedule([byte for subkey in key for byte in subkey])

# Раунд шифрування
aes_round(plaintext, round_keys[0:4])

print("Ciphertext after one round:")
for row in plaintext:
    print(row)
